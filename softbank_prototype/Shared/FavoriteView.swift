...
class FavoriteView: UIView {
    @IBOutlet weak var button: UIButton! {
        didSet {
            button.setImage(R.image.star()!.withRenderingMode(.alwaysTemplate), for: UIControlState.normal.union(.highlighted))
            button.setImage(R.image.star()!.withRenderingMode(.alwaysTemplate), for: UIControlState.selected.union(.highlighted))
        }
    }
    
    @IBInspectable
    var favoriteText: String? = nil {
        didSet {
            button?.setTitle(favoriteText, for: .normal)
            button?.setTitle(favoriteText, for: UIControlState.normal.union(.highlighted))
        }
    }
    @IBInspectable
    var favoritedText: String? = nil {
        didSet {
            button?.setTitle(favoritedText, for: .selected)
            button?.setTitle(favoritedText, for: UIControlState.selected.union(.highlighted))
        }
    }
    ...
    private func loadNib() {
        guard let view = Bundle(for: type(of: self)).loadNibNamed(R.nib.favoriteView.name, owner: self, options: nil)?.first as? UIView else {
            return
        }
