...
class GoodView: UIView {
    @IBOutlet weak var button: UIButton! {
        didSet {
            button.setImage(R.image.good()!.withRenderingMode(.alwaysTemplate), for: UIControlState.normal.union(.highlighted))
            button.setImage(R.image.good()!.withRenderingMode(.alwaysTemplate), for: UIControlState.selected.union(.highlighted))
        }
    }
    
    @IBInspectable
    var goodText: String? = nil {
        didSet {
            button?.setTitle(goodText, for: .normal)
            button?.setTitle(goodText, for: UIControlState.normal.union(.highlighted))
        }
    }
    @IBInspectable
    var goodedText: String? = nil {
        didSet {
            button?.setTitle(goodedText, for: .selected)
            button?.setTitle(goodedText, for: UIControlState.selected.union(.highlighted))
        }
    }
    var isGooded: Bool {
        get {
...
//    }
    ...
    private func loadNib() {
        guard let view = Bundle(for: type(of: self)).loadNibNamed(R.nib.goodView.name, owner: self, options: nil)?.first as? UIView else {
            return
        }
